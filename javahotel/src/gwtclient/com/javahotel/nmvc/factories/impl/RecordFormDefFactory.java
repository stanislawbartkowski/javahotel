...
public class RecordFormDefFactory implements IFormTitleFactory, IFormDefFactory {
    ...
    public FormLineContainer construct(IDataType dType) {
        ...
        if (dd.isDictType()) {
            ...
            case VatDict:
                fList = getDict(false);
                fList.add(new FormField("Procent", new VField(
            ...
            case ServiceDict:
                ...
                fList.add(new FormField("Typ", servT));
                break;

            case RoomObjects:
                fList = getDict(false);
                fList.add(new FormField("Standard", new VField(
                        ResObjectP.F.standard)));
            ...
            case OffSeasonDict:
                fList = getDict(false);
                fList.add(new FormField("Od", new VField(OfferSeasonP.F.startp)));
                fList.add(new FormField("Do", new VField(OfferSeasonP.F.endp)));
                break;

            case CustomerList:
                fList = getDict(true);
                fList.add(new FormField("Nazwa 1",
                        new VField(CustomerP.F.name1)));
                fList.add(new FormField("Nazwa 2",
                        new VField(CustomerP.F.name2)));
                fList.add(new FormField("Rodzaj", new VField(CustomerP.F.cType)));

                fList.add(new FormField("Pan/Pani", new VField(
                        CustomerP.F.pTitle)));
                fList.add(new FormField("Imię", new VField(
                        CustomerP.F.firstName)));
                fList.add(new FormField("Nazwisko", new VField(
                        CustomerP.F.lastName)));
                fList.add(new FormField("PESEL", new VField(CustomerP.F.PESEL)));
                fList.add(new FormField("Rodzaj dokumentu", new VField(
                        CustomerP.F.docType)));
                fList.add(new FormField("Numer dokument", new VField(
                        CustomerP.F.docNumber)));

                fList.add(new FormField("Kraj", new VField(CustomerP.F.country)));
                fList.add(new FormField("Kod pocztowy", new VField(
                        CustomerP.F.zipCode)));
                fList.add(new FormField("Miasto", new VField(CustomerP.F.city)));
                fList.add(new FormField("Adres 1", new VField(
                        CustomerP.F.address1)));
                fList.add(new FormField("Adres 2", new VField(
