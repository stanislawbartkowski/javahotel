...
class ScrollTable implements IScrollSeason {
    ...
    private int startno;
    private final IFormLineView edP;
    ...
    ScrollTable(final IResLocator pLoc, final IDrawPartSeason i, final int pNo) {
        ...
        edP = eFactory.constructLabelTextEdit(Empty.getFieldType(), "Liczba");
        edP.setValObj("" + pNo);
    }
     ...
     */
    private int getMno() {
        String va = (String) edP.getValObj();
        int no = CUtil.getNumb(va);
        return no;
    }
    ...
    private void setUPanel(final HorizontalPanel vvP) {
        ...
        uP.add(edP.getGWidget());
        ii.setW(new GWidget(uP));
        // ii.setSWidget(uP);
        HasVerticalAlignment.VerticalAlignmentConstant al = HasVerticalAlignment.ALIGN_BOTTOM;
        uP.setCellVerticalAlignment(edP.getGWidget(), al);
    }
