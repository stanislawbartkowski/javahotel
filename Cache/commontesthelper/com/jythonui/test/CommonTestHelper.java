...
abstract public class CommonTestHelper {
	...
	protected void equalB(double f, BigDecimal b, int afterdot) {
		BigDecimal c = new BigDecimal(f).setScale(afterdot, BigDecimal.ROUND_HALF_UP);
	...
	protected void runAction(DialogVariables v, String dialogName, String actionId) {
		iServer.runAction(new RequestContext(), v, dialogName, actionId);
	}

	protected void runAction(String token, DialogVariables v, String dialogName, String actionId, String locale) {
		RequestContext req = new RequestContext();
		req.setToken(token);
	...
	protected void runAction(String token, DialogVariables v, String dialogName, String actionId) {
	...
	protected boolean buttContainsAttr(DialogInfo i, String field, String attr) {
		ButtonItem ii = DialogFormat.findE(i.getDialog().getButtonList(), field);
		assertNotNull(ii);
		return ii.isAttr(attr);
	}

	protected boolean leftbuttContainsAttr(DialogInfo i, String field, String attr) {
		ButtonItem ii = DialogFormat.findE(i.getDialog().getLeftButtonList(), field);
		assertNotNull(ii);
		return ii.isAttr(attr);
	}

	protected boolean listContainsAttr(DialogInfo i, String list, String col, String attr) {
