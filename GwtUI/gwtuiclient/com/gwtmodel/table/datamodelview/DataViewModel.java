...
class DataViewModel extends AbstractSlotContainer implements IDataViewModel {
	...
	private class FormChangeListener implements IFormChangeListener {
		...
		public void onChange(IFormLineView i, boolean afterFocus) {
			...
			SlU.publishValueChange(dType, DataViewModel.this, fie, i, afterFocus);
		}
	}
	...
	DataViewModel(GwtFormViewFactory gFactory, IDataType dType, FormLineContainer fContainer,
			IDataModelFactory dFactory, IDataFormConstructorAbstractFactory abFactory) {
		syP = new SyPublish();
		...
		registerSubscriber(dType, DataActionEnum.ChangeViewFormToInvalidAction, new InvalidateMess());
		registerSubscriber(dType, DataActionEnum.ChangeViewFormModeAction, new ChangeMode());
		registerSubscriber(dType, DataActionEnum.DrawViewFormAction, new DrawModel());
		registerSubscriber(dType, DataActionEnum.ClearViewFormAction, new ClearAction());
		registerSubscriber(SignalSetHtmlId.constructSlot(dType), new SetHtmlId());
		registerSubscriber(SignalChangeMode.constructSlot(dType), new CustomChangeMode());
		registerSubscriber(SendPanelElemSignal.constructSlotSendPanelElem(dType), new GetElemPanel());

		registerCaller(dType, GetActionEnum.GetViewModelEdited, new GetterModel());
		registerCaller(dType, GetActionEnum.GetFormFieldWidget, new GetterWidget());
		registerCaller(dType, GetActionEnum.GetEditContainer, new GetterContainer());
		for (FormField fie : fContainer.getfList()) {
			IFormLineView vie = fie.getELine();
