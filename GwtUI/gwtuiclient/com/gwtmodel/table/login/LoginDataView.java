...
class LoginDataView extends AbstractSlotMediatorContainer implements ILoginDataView {
	...
	LoginDataView(CellId panelId, IDataType dType, FormLineContainer lContainer, IDataModelFactory dFactory,
			IDataValidateAction vAction) {
		this.dFactory = dFactory;
		this.dType = dType;
		ListOfControlDesc bControl = tFactories.getControlButtonFactory().constructLoginButton();
		IControlButtonView bView = ControlButtonViewFactory.construct(dType, bControl, false);
		IPanelView pView = PanelViewFactory.construct(dType, panelId);
		CellId controlId = pView.addCellPanel(1, 0);
		CellId cellTableId = pView.addCellPanel(0, 0);
		pView.createView();
		IDataViewModel daView = tFactories.getdViewFactory().construct(dType, lContainer, dFactory);
		...
		slMediator.getSlContainer().registerCaller(slFactory.construct(dType, GetActionEnum.GetViewComposeModelEdited),
				new GetCompose());
		slMediator.getSlContainer().registerRedirector(
				slFactory.construct(dType, new ClickButtonType(ClickButtonType.StandClickEnum.ACCEPT)),
