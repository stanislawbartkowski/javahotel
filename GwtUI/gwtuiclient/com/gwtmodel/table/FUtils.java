...
public class FUtils {
	...
	private static int compTimestamp(IVModelData row, IVField f, IVModelData filter, IVField from) {
	...
	private static int compInteger(IVModelData row, IVField f, IVModelData filter, IVField from) {
		Integer rB = getValueInteger(row, f);
		Integer fB = getValueInteger(filter, from);
		return fB.compareTo(rB);
	}

	public static int compareValue(IVModelData row1, IVField f1, IVModelData row2, IVField f2, boolean ignorecase) {
	...
	private static class TempV extends AbstractVField {
	...
	public static boolean inRange(IVModelData row, IVField f, IVModelData filter, IVField from, IVField to,
		...
		if (!emptyfrom) {
			...
			case BOOLEAN:
				...
				break;
			default:
				compfrom = compString(row, f, filter, from, true, true, checkEqual || !emptyto);
				break;
			}
		}
		if (!emptyto) {
	...
	public static String getValueOS(Object oo, IVField f) {
		...
		try {
			...
			case BOOLEAN:
				return getBoolS(o, f);
			default:
				return ConvertTT.toS(o, f.getType().getType(), f.getType().getAfterdot());
			}
	...
	public static String constructAssertS(Object o, @SuppressWarnings("rawtypes") Class cl) {
