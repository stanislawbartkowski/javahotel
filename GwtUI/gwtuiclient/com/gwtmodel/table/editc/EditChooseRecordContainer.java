...
class EditChooseRecordContainer extends AbstractSlotMediatorContainer implements IEditChooseRecordContainer {
	...
	private void modifList(PersistTypeEnum e) {
		slMediator.getSlContainer().publish(dType, DataActionEnum.ChangeViewComposeFormModeAction, e);
		slMediator.getSlContainer().publish(dType, new ClickButtonType(LIST_BUTTON),
				new ButtonAction(e.readOnly() ? ButtonAction.Action.DisableButton : ButtonAction.Action.EnableButton));
	}
	...
	EditChooseRecordContainer(ICallContext iContext, IDataType publishdType, boolean withoutForm) {
		...
		slMediator.getSlContainer().registerSubscriber(dType, sChoose, new ChooseC(dType, slMediator));
		IControlButtonView bView = ControlButtonViewFactory.construct(dType, cList, false);
		IGetViewControllerFactory fa = GwtGiniInjector.getI().getTableFactoriesContainer()
