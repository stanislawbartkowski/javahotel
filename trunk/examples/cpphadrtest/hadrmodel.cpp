...
std::string readSqlError(std::string info,SQLSMALLINT type,SQLHANDLE handle) {
...
std::string readSqlError(std::string info,SQLHANDLE handle) {
    return readSqlError(info,SQL_HANDLE_DBC,handle);
}
...
public:
	...
	errorHandleSTMT = true;
        ...
        if (!SQL_SUCCEEDED(RetCode)) {
            if (errorHandleSTMT) {
	      ha.lastActionStatus = readSqlError(info,SQL_HANDLE_STMT,StmtHandle);
	    }
	    else {
	      ha.lastActionStatus = readSqlError(info,SQL_HANDLE_DBC,ha.getConHandle());
	    }
        } else {
...
void hadrmodel::connect()
{
    ...
    if (SQL_SUCCEEDED(RetCode)) {
        ...
        lastActionStatus = readSqlError("Connecting to database",ConHandle);
        ConHandle = 0;
    }
