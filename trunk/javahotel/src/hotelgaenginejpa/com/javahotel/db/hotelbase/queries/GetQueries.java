...
public class GetQueries {
	...
	public static Collection<IId> getDList(final ICommandContext iC,
		...
		switch (d) {
		case PriceListDict:
			Collection<OfferPrice> colx = iC.getJpa().getAllList(
					OfferPrice.class);
	...
	public static int getNumber(final ICommandContext iC, final Class<?> cla) {
		Collection<?> col = iC.getJpa().getAllList(cla);
		return col.size();
	}
	...
	public static Collection<PaymentRow> getPaymentForReservation(
			...
			final String oName) {
		Collection<PaymentRow> cc = new ArrayList<PaymentRow>();

		Collection<Booking> cB = iC.getJpa().getListQuery(Booking.class,
				"hotelId", iC.getRHotel().getId().getL());
		for (Booking b : cB) {
			Collection<BookRecord> cBo = b.getBookrecords();
			for (BookRecord bo : cBo) {
				Collection<BookElem> cE = bo.getBooklist();
				for (BookElem be : cE) {
					if (!be.getResObject().equals(oName)) {
						continue;
					}
					Collection<PaymentRow> cR = be.getPaymentrows();
					for (PaymentRow p : cR) {
	...
	public static Collection<AdvancePayment> getValidationForHotel(
		...
		Collection<AdvancePayment> col = iC.getJpa().getAllList(
				AdvancePayment.class);
		Collection<AdvancePayment> out = new ArrayList<AdvancePayment>();
	...
	public static Collection<ParamRegistry> getRegistryEntries(
		...
		Collection<ParamRegistry> c = iC.getJpa().getListQuery(
				ParamRegistry.class, "hotelId", iC.getRHotel().getId().getL());
		Collection<ParamRegistry> out = new ArrayList<ParamRegistry>();
