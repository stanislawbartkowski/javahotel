...
class DrawResGrid {
	...
	private class SetObjectRowState implements ISignal {
		...
		SetObjectRowState(final Collection<String> r, final Collection<Date> d) {
			this.rList = r;
			this.dLine = d;
		}
	...
	void draw() {
		Collection<? extends AbstractTo> list = resList.getResList();
		Collection<String> rList = new ArrayList<String>();
		for (final AbstractTo p : list) {
			ResObjectP pO = (ResObjectP) p;
			rList.add(pO.getName());
		}
		Collection<Date> dLine = pCa.getDLine();
