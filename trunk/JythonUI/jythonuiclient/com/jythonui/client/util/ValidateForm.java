...
public class ValidateForm {
    ...
    private static FieldItem findId(ValidateRule va, String aid, DialogFormat d) {
        String id = getAttr(va, aid);
        if (id == null) {
            return null;
        }
        FieldItem f = d.findFieldItem(id);
        if (f == null) {
    ...
    public static boolean validateV(IDataType dType, ISlotable iSlo,
        ...
        if (VerifyEmpty.isEmpty(dType, v, iSlo, d.getFieldList(), errSig)) {
            return false;
        }
        // check validate rules
        List<InvalidateMess> errMess = new ArrayList<InvalidateMess>();
        for (ValidateRule va : d.getValList()) {
            ...
            Op o = Op.valueOf(op);
            FieldItem f1 = findId(va, ICommonConsts.ID, d);
            FieldItem f2 = findId(va, ICommonConsts.VALIDATEID1, d);
            if (f1 == null || f2 == null) {
                return false;
            }
            ...
            errMess.add(new InvalidateMess(v1, displayN));
        } // for
        if (!errMess.isEmpty()) {
