...
function printlf() { print("") }

function printline(l) { print(l) }

function printcurrent() { printline($0) }

function outputline() {
  if (PRINT == 1) printcurrent()
}

function printeo() { printline("@"); printlf()  }
...
function recognizeobject(objtype) {
  ...
  else if (objtype == "FOREIGN") OBJTYPE=5
  else if (objtype == "TYPES") OBJTYPE=6
  else if (objtype == "PACKAGE") OBJTYPE=7
  else if (objtype == "PACKAGEBODY") OBJTYPE=8
  else if (objtype == "SEQUENCE") OBJTYPE=9
  else if (objtype == "ALL") OBJTYPE=10
  else {
    print "Invalid objtype value:",objtype,"Expected : TABLE, VIEW, TRIGGER, PROCEDURE, FUNCTION, FOREIGN, TYPES, PACKAGE, PACKAGEBODY, SEQUENCE or ALL"
    exit
  }  
  return OBJTYPE
}
