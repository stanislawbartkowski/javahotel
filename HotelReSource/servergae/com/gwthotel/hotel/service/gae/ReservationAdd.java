...
class ReservationAdd {
	...
	ReservationAdd(OObjectId hotel, ReservationForm elem, boolean change, ICrudObjectGenSym iGen) {
		this.elem = elem;
		this.hotel = hotel;
	...
	void beforeAdd() {
		...
		if (change)
			deList = DictUtil.findResDetailsForRes(ho, elem.getName(), ServiceType.HOTEL);
		else
	...
	void addTran() {
		...
		final List<EResDetails> eRes = DictUtil.toED(ho, null, elem.getResDetail());
