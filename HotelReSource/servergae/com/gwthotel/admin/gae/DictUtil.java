...
public class DictUtil extends UtilHelper {
	...
	public static <E> E findEE(EObject eh, String name, Class<E> cl) {
		...
		if (e != null)
			return e;
		errorMess(L(), IErrorCode.ERRORCODE110, ILogMess.OBJECTBYNAMECANNOTBEFOUND, name, eh.getName());
		return null;
	}
	...
	public static List<EResDetails> findResDetailsForRes(EObject ho, String resName, ServiceType serviceType) {
		List<EResDetails> li = ofy().load().type(EResDetails.class).ancestor(ho).filter("resName == ", resName).list();
	...
	public static ReservationPaymentDetail toResD(EResDetails d) {
	...
	public static void toRP(List<ReservationPaymentDetail> resList, List<EResDetails> rList) {
		for (EResDetails d : rList)
			resList.add(toResD(d));
	}
	...
	public static EResDetails toEResDetail(EObject ho, EHotelReservation e, ReservationPaymentDetail r) {
	...
	public static List<EResDetails> toED(EObject ho, EHotelReservation e, List<ReservationPaymentDetail> rList) {
	...
	public static List<ECustomerBill> findBillsForRese(EObject ho, String resName) {
		List<ECustomerBill> re = ofy().load().type(ECustomerBill.class).ancestor(ho).filter("resName ==", resName)
				.list();
		return re;
	}

	public static List<EBillPayment> findPaymentsForBill(EObject ho, String billName) {
		List<EBillPayment> li = ofy().load().type(EBillPayment.class).ancestor(ho).filter("billName == ", billName)
				.list();
		return li;
	}
