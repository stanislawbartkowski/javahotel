...
public class jTPCC extends JFrame implements jTPCCConfig, ActionListener, WindowListener
{
    ...
    private String getDefault(String sysP, String defa) {
        String val = System.getProperty(sysP);
        return val != null ? val : defa;
    }
    
    private boolean defRadioTime() {
        String NoPT = System.getProperty(defaultTransactionsPerTerminalP);
        String NoM = System.getProperty(defaultMinutesP);
        if ((NoPT == null) && (NoM == null)) { return true; }
        return NoM != null ? true : false;
    }
    ...
    private long newOrderCounter, sessionStartTimestamp, sessionEndTimestamp, sessionNextTimestamp=0, sessionNextKounter=0, sessionLastTimeStamp;
    ...
    System.out.println("password=******");
        ...
        jTextFieldNumTerminals = new JTextField(getDefault(defaultNumTerminalsP,"1"));
        ...
        jTextFieldNumWarehouses = new JTextField(getDefault(defaultNumWarehousesP,"1"));
        ...
        jTextFieldMinutes = new JTextField(getDefault(defaultMinutesP,"1"));
        ...
        jTextFieldTransactionsPerTerminal = new JTextField(getDefault(defaultTransactionsPerTerminalP,"1"));
        ...
        jRadioButtonTime = new JRadioButton("Minutes", defRadioTime());
        buttonGroupTimeNum.add(jRadioButtonTime);
        jPanelConfigTerminals22.add(jRadioButtonTime);
        jRadioButtonNum = new JRadioButton("Transactions per terminal", !defRadioTime());
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        if(e.getSource() == jButtonStartTransactions)
        {
            ...
            sessionLastTimeStamp = sessionStartTimestamp;
    ...
    private void endReport()
    {
        try
        {
            printStreamReport.println("\n\nFinal\t=60000*" + newOrderCounter + "/" + (sessionEndTimestamp - sessionStartTimestamp));
            printStreamReport.println("\nMeasured tpmC\t=" + countTPC(newOrderCounter,0,sessionEndTimestamp,sessionStartTimestamp));
    ...
    private int countTPC(long currentOrderCounter, long prevOrderCounter,long currentMilis,long prevMilis) {
       if (currentMilis == prevMilis) {
           return 0;
       }
       double tpmC = (6000000*(currentOrderCounter-prevOrderCounter)/(currentMilis - prevMilis))/100.0;
       return (int) tpmC;        
    }
...
//            double tpmC = (6000000*fastNewOrderCounter/(currTimeMillis - sessionStartTimestamp))/100.0;
            int tpmC = countTPC(fastNewOrderCounter,0,currTimeMillis,sessionStartTimestamp);
...
//            recentTpmC = (fastNewOrderCounter - sessionNextKounter) * 12; 
            recentTpmC = countTPC(fastNewOrderCounter,sessionNextKounter,currTimeMillis,sessionLastTimeStamp);
            sessionNextKounter = fastNewOrderCounter;
            sessionNextTimestamp += 5000;  /* check this every 5 seconds */
            sessionLastTimeStamp = currTimeMillis;
        }
