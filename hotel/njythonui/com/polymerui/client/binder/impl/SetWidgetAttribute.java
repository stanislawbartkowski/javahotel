 ...
 */
package com.polymerui.client.binder.impl;
...
import com.polymerui.client.view.util.ItemsEvent;
...
public class SetWidgetAttribute implements ISetWidgetAttribute {
	...
	private WidgetTypes widgetToType(Widget w) {
		...
		try {
			...
			Utils.errAlertB(typeName, M.M().WidgetToSetAttrNotImplemented());
			return null;
		}
	}
	...
	public void setAttr(Widget w, String attr, String val) {
		...
		if (BinderWidgetAttributes.isBinderWidgetAttr(wType)) {
			...
			if (a == null)
				Utils.errAlertB(widgetToType(w).toString(), M.M().AttributeNotRecognized(attr, val));
			if (a.errval() != null)
				Utils.errAlertB(widgetToType(w).toString(), M.M().InvalidValueForAttribute(attr, val));
			if (a.getString() != null)
