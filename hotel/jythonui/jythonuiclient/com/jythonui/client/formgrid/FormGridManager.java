 ...
 */
package com.jythonui.client.formgrid;
...
class FormGridManager implements IFormGridManager {
	...
	FormGridManager(IDialogContainer dContainer, IDataType publishType) {
		this.dContainer = dContainer;
		this.publishType = publishType;
	}

	@Override
	public void addDataType(String id, IDataType dType) {
		sData.put(id, dType);
	}
	...
	@Override
	public ISlotable constructSlotable(String id) {
		DialogFormat d = dContainer.getD();
		CheckList cCheck = d.findCheckList(id);
		assert cCheck != null;
		IGridView gView;
		if (cCheck.isBoolean())
			gView = GridViewFactory.constructBoolean(true, true, true);
		else
			gView = GridViewFactory.constructDecimal(true, true, true);
		return new GridSlotable(gView, id);
	}

	private void readLines(List<String> idS, List<String> dispS, CheckListElem cElem, ListOfRows lRows) {
	...
	@Override
	public void addLinesAndColumns(String id, DialogVariables v) {
		DialogFormat d = dContainer.getD();
		for (String s : v.getCheckVariables().keySet()) {
			...
			if (!c.getLines().getRowList().isEmpty() && !c.getColumns().getRowList().isEmpty()) {
				RowsCols rCols = new RowsCols(s);
				List<String> linesNames = new ArrayList<String>();
				List<String> colsNames = new ArrayList<String>();
				readLines(rCols.lines, linesNames, cCheck.getLines(), c.getLines());
				readLines(rCols.cols, colsNames, cCheck.getColumns(), c.getColumns());
				rMap.put(s, rCols);
			...
			for (String row : c.getVal().keySet()) {
				...
				for (RowContent r : lRows.getRowList()) {
					String col = rI.get(r, cCheck.getLines().getId()).getValueS();
					int colL = rCol.findCol(col);
					if (bView != null) {
						Boolean val = rI.get(r, ICommonConsts.CHECKLINEVALUE).getValueB();
						bView.setRowBoolean(rowL, colL, val);
					} else {
						BigDecimal b = rI.get(r, ICommonConsts.CHECKLINEVALUE).getValueBD();
	...
	private boolean checkError(String checkId, DialogVariables v) {
		...
		for (RowContent r : c.getErrors().getRowList()) {
			...
			String errmess = rI.get(r, ICommonConsts.CHECKERRORMESS).getValueS();
	...
	@Override
	public void modifAttr(String checkId, String action, FieldValue val) {
	...
	@Override
	public boolean okGridErrors(DialogVariables v) {
		boolean ok = true;
		for (String s : gData.keySet()) {
			if (checkError(s, v))
				ok = false;
