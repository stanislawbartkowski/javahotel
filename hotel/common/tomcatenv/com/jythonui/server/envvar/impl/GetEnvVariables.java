...
public class GetEnvVariables extends UtilHelper implements IGetEnvVariable {
	...
	@Inject
	public GetEnvVariables(@Named(ISharedConsts.JYTHONMESSSERVER) IGetLogMess gMess, ICommonCacheFactory cFactory) {
		this.gMess = gMess;
	...
	private EnvVar getEnvStringDirect(String tName, ResType rType, boolean throwerror) {
		Context initCtx;
		EnvVar r = new EnvVar();
		String eName = tName;
		String conte = null;
		...
		try {
			...
			if (throwerror)
				errorLog(gMess.getMess(IErrorCode.ERRORCODE43, ILogMess.ERRORWHILEREADINGCONTEXT, eName), e);
			// ENVVARIABLENOTFOUND
