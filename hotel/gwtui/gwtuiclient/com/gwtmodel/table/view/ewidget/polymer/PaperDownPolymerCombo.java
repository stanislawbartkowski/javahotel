...
class PaperDownPolymerCombo extends AbstractWField implements IValueLB {
	...
	private void setEvent() {
		...
		Polymer.ready(pDown.getElement(), arg -> {
	...
	protected PaperDownPolymerCombo(IVField v, IFormFieldProperties pr, IGetDataList iGet, boolean addEmpty) {
		super(v, pr, null);
	...
	public void replaceWidget(Widget w) {
		if (!(w instanceof PaperDropdownMenu))
			Utils.errAlertB(LogT.getT().ReplaceTypeNotCorrect(WidgetTypes.PaperDropdownMenu.name(),
					PaperDropdownMenu.class.getName(), w.getClass().getName()));
