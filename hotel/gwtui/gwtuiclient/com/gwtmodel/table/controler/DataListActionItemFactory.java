...
class DataListActionItemFactory {
	...
	DataListActionItemFactory(TablesFactories tFactories, IDataType dType, ISlotable iSlo, DataListParam listParam,
	...
	private class AfterPersistData extends Signaller {
		...
		public void signal(ISlotSignalContext slContext) {
			hide();
			if (iSlo != null) {
				iSlo.getSlContainer().publish(dType, DataActionEnum.RefreshAfterPersistActionSignal, persistTypeEnum);
			}
		}
	}
	...
	static private class FormDialog extends ModalDialog {
		...
		FormDialog(VerticalPanel vp, String title, IGWidget w, boolean modal, ISignal aClose) {
	...
	static class DrawForm implements ISlotListener {
		...
		private final IGetStandardMessage iMess = GwtGiniInjector.getI().getStandardMessage();

		DrawForm(WSize wSize, String title, SolidPos sPos, ClickButtonType.StandClickEnum action, boolean modal,
		...
		public void signal(ISlotSignalContext slContext) {
			...
			if (CUtil.EmptyS(title))
				...
				titleDialog = LogT.getT().formRecordTitle(iMess.getMessage(title), addTitle);
	...
	static class ResignAction extends Signaller {
		...
		public void signal(ISlotSignalContext slContext) {
			...
			if (pAsk != null) {
				...
				// yesD.show(w);
				HelperDialogFactory.constructyes(pAsk, null, yes).show(new WSize(w));
				return;
	...
	private class PersistData implements ISlotListener {
		...
		PersistData(PersistTypeEnum persistTypeEnum, IComposeController iController, DataActionEnum dataActionEnum,
				String ask) {
			this.persistTypeEnum = persistTypeEnum;
		...
		private void goPersist(ISlotSignalContext slContext) {
			SlU.publishActionPersist(dType, iController, slContext, dataActionEnum, persistTypeEnum);
		}
	...
	private class GetterModel implements ISlotCallerListener {
		...
		public GetterModel(SlotListContainer slControlerContainer, IVModelData peData) {
		...
		public ISlotSignalContext call(ISlotSignalContext slContext) {
			...
			IVModelData pData = slControlerContainer.getGetterIVModelData(dType, GetActionEnum.GetViewModelEdited,
					peData);
	...
	private class GetListLine implements ISlotCallerListener {
		...
		@Override
		public ISlotSignalContext call(ISlotSignalContext slContext) {
			return iSlo.getSlContainer().getGetterContext(dType, GetActionEnum.GetListLineChecked);
		}
	}

	IComposeController BoxActionItem(ClickButtonType.StandClickEnum action, PersistTypeEnum persistTypeEnum,
			IVModelData peData, WSize wSize, boolean contentOnly) {
		...
		if (!contentOnly) {
			IControlButtonView cView = ControlButtonViewFactory.construct(dType, liControls, false);
			...
			if (addModifAction) {
				aClose = new ISignal() {
					...
					@Override
					public void signal() {
						slControlerContainer.publish(dType, new ClickButtonType(ClickButtonType.StandClickEnum.RESIGN),
								new ButtonAction(ButtonAction.Action.ForceButton));
					}
				};
			}
			DrawForm dForm = new DrawForm(wSize, title, listParam.getFormFactory().getSolidPos(iCall), action, true,
					null, aClose);
			slControlerContainer.registerSubscriber(dType, cId, dForm);
			String resignAsk = listParam.getMenuOptions().getAskString(BoxActionMenuOptions.ASK_BEFORE_RESIGN);
			String persistAsk = listParam.getMenuOptions().getAskString(BoxActionMenuOptions.ASK_BEFORE_PERSIST);
			ResignAction aRes = new ResignAction(dForm, null, resignAsk);

			slControlerContainer.registerSubscriber(listParam.getMenuOptions().constructRemoveFormDialogSlotType(),
					aRes);

			// redirect Resign (default: remove without any question)
			slControlerContainer.registerRedirector(listParam.getMenuOptions().constructResignButtonSlotType(dType),
					listParam.getMenuOptions().getSlotType(BoxActionMenuOptions.REDIRECT_RESIGN));

			PersistData pData = new PersistData(persistTypeEnum, fController, DataActionEnum.ValidateComposeFormAction,
					persistAsk);
			if (action == ClickButtonType.StandClickEnum.SHOWITEM) {
				slControlerContainer.registerSubscriber(dType, ClickButtonType.StandClickEnum.ACCEPT, aRes);
			} else {
				slControlerContainer.registerSubscriber(dType, ClickButtonType.StandClickEnum.ACCEPT, pData);
			}

			pData = new PersistData(persistTypeEnum, fController, DataActionEnum.PersistComposeFormAction, persistAsk);
			slControlerContainer.registerSubscriber(dType, DataActionEnum.ValidSignal, pData);

			slControlerContainer.registerSubscriber(dType, DataActionEnum.PersistDataSuccessSignal,
					new AfterPersistData(dForm, persistTypeEnum));

			fController.startPublish(cId);
			slControlerContainer.registerCaller(dType, GetActionEnum.GetGWidget, new GetterGWidget(dForm));
		} else {
			fController.createComposeControler(cId);
		}
		slControlerContainer.publish(dType, DataActionEnum.DrawViewComposeFormAction, peData, persistTypeEnum);
		slControlerContainer.publish(dType, DataActionEnum.DefaultViewComposeFormAction, peData, persistTypeEnum);

		slControlerContainer.publish(dType, DataActionEnum.ChangeViewComposeFormModeAction, persistTypeEnum);

		slControlerContainer.registerCaller(dType, GetActionEnum.GetModelToPersist,
				new GetterModel(slControlerContainer, peData));
		slControlerContainer.registerCaller(dType, GetActionEnum.GetListLineChecked, new GetListLine());
		return fController;
	}
	...
	private class ActionItem implements ISlotListener {
		...
		public void signal(ISlotSignalContext slContext) {
			...
			ISlotSignalContext ret = iSlo.getSlContainer().getGetterContext(dType, GetActionEnum.GetListLineChecked);
			...
			if (peData != null) {
				listParam.getDataFactory().copyFromPersistToModel(dType, peData, mModel);
			} else {
	...
	private void publishP(ButtonAction.Action b, boolean redirectB) {
		...
		iSlo.getSlContainer().publish(dType, new ClickButtonType(ClickButtonType.StandClickEnum.ADDITEM),
				new ButtonAction(b));

		if (redirectB) {
			iSlo.getSlContainer().publish(dType, new ClickButtonType(ClickButtonType.StandClickEnum.MODIFITEM),
		...
		iSlo.getSlContainer().publish(dType, new ClickButtonType(ClickButtonType.StandClickEnum.REMOVEITEM),
				new ButtonAction(b));
	}
	...
	private class ChangeMode implements ISlotListener {
		...
		public void signal(ISlotSignalContext slContext) {
			...
			publishP(persistTypeEnum.readOnly() ? ButtonAction.Action.DisableButton : ButtonAction.Action.EnableButton,
