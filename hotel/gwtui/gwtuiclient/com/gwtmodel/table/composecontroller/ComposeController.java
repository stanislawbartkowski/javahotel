...
class ComposeController extends AbstractSlotMediatorContainer implements IComposeController {
	...
	private class EditCallerGetter implements ISlotCallerListener {
		...
		public ISlotSignalContext call(ISlotSignalContext slContext) {
			...
			for (ComposeControllerType cType : cList) {
				...
				IVModelData ppData = slMediator.getSlContainer().getGetterIVModelData(cType.getdType(), getA, pData);
				if (ppData != null) {
					pData = ppData;
				}
			}
			return slMediator.getSlContainer().getGetterContext(slContext.getSlType(), pData);
		}
	}
	...
	private class DrawAction implements ISlotListener {
		...
		public void signal(ISlotSignalContext slContext) {
			for (ComposeControllerType cType : cList) {
				...
				slMediator.getSlContainer().publish(cType.getdType(), dataActionEnum, slContext);
			}
			if (dataActionEnum == DataActionEnum.DrawViewFormAction) {
				slMediator.getSlContainer().publish(dType, DataActionEnum.AfterDrawViewFormAction, slContext);
			}
			if (dataActionEnum == DataActionEnum.ChangeViewFormModeAction) {
				slMediator.getSlContainer().publish(dType, DataActionEnum.AfterChangeModeFormAction, slContext);
			}
		}
	...
	private class ComposeValidateAction implements ISlotListener {
		...
		@Override
		public void signal(ISlotSignalContext slContext) {
			ISlotSignalContext rSl = slContextFactory.construct(sldType, slContext);
	...
	private class SubValidate implements ISlotListener {
		...
		void publish(ISlotSignalContext slContext) {
			ISlotSignalContext rSl = slContextFactory.construct(slType, slContext);
	...
	public void createComposeControler(CellId cellId) {
		pView = PanelViewFactory.construct(dType, cellId);
		...
		slMediator.getSlContainer().registerSubscriber(dType, DataActionEnum.DrawViewComposeFormAction,
				new DrawAction(DataActionEnum.DrawViewFormAction));
		slMediator.getSlContainer().registerSubscriber(dType, DataActionEnum.ChangeViewComposeFormModeAction,
				new DrawAction(DataActionEnum.ChangeViewFormModeAction));
		slMediator.getSlContainer().registerSubscriber(dType, DataActionEnum.DefaultViewComposeFormAction,
				new DrawAction(DataActionEnum.DefaultViewFormAction));

		slMediator.getSlContainer().registerRedirector(
				slFactory.construct(dType, DataActionEnum.PersistComposeFormAction),
				slFactory.construct(dType, DataActionEnum.PersistDataAction));

		slMediator.getSlContainer().registerRedirector(slFactory.construct(dType, DataActionEnum.InvalidSignal),
				slFactory.construct(dType, DataActionEnum.ChangeViewFormToInvalidAction));

		int no = 0;
		List<SubValidate> li = new ArrayList<SubValidate>();
		SlotType sldType = slFactory.construct(dType, DataActionEnum.ValidateAction);
		for (ComposeControllerType c : cList) {
			IDataType dType = c.getdType();
			if (dType == null) {
				continue;
			}
			SlotType slType = slFactory.construct(dType, DataActionEnum.ValidateAction);
			...
			if (i > 0) {
				...
				SlotType slVal = slFactory.construct(dType, DataActionEnum.ValidSignal);
				c.getiSlot().getSlContainer().registerSubscriber(slVal, va);
				li.add(va);
			}
		}
		slMediator.getSlContainer().registerSubscriber(dType, DataActionEnum.ValidateComposeFormAction,
				new ComposeValidateAction(no, li, sldType));

		slMediator.getSlContainer().registerCaller(dType, GetActionEnum.GetViewComposeModelEdited,
				new EditCallerGetter(GetActionEnum.GetViewModelEdited));
		slMediator.getSlContainer().registerCaller(dType, GetActionEnum.GetComposeModelToPersist,
